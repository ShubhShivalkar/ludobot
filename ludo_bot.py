...
class case_score():
    ...
    def in_enemy_radar(self):
        ...
        for i in range(4):
            for j in range(4):
                if 0 < self.player[i] - self.opponent[i]  <= 6:
                    if self.player[i] + self.dice < self.opponent[i] + 6:
        ...
        for i in range(4):
            for j in range(4):
                if 0 < self.opponent[j] - self.player[i] <= 6:
                    if (self.player[i] + self.dice == self.opponent[j]) | (self.opponent[j] - self.player[i] + self.dice <= 6): 
    ...
    def token_at_start(self):
        ...
        for i in range(4):
            if self.isZero(self.player[i]):
                start_token_arr[i] += self.count * 5
    ...
    def token_in_home_lane(self):
        ...
        home_lane_arr = [0]*4
        for i in range(4):
            if 56 - self.player[i] < 6:
                if self.player[i] +  self.dice == 56:
    ...
    def return_score(self):
        ...
        total_score = [0]*4
        for i in range(4):
            total_score[i] = self.in_enemy_radar()[i] * w1 + self.in_player_radar()[i] * w2
            + self.token_at_start()[i] * w5 + self.token_in_home_lane()[i] * w6 + self.player[i] * w7
        return total_score
    ...
    def opponent_move(self):
        if len(self.move_pieces):
            ...
            #print(self.move_pieces)
            #print(random.choice(self.move_pieces))
            print(self.move_pieces)
...
The Ludo cube has six sides, but eyes 3 and 5 have been replaced with globes and stars. If you hit a globe, you must move a piece to the nearest globe space on the playing board – also from your starting area. If you hit a star with the dice, you must move one piece forward to the nearest star field - and from there jump to the next one. If a piece is too close to the target to benefit from a globe or star, the move cannot be used on that piece. All other rolls give the right to move a piece the number of squares shown by the eyes. Extra throws are awarded by hitting a globe or a six.
